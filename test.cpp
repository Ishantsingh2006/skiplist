...
void test_basic_operations() {
    ...
    skiplist<int, std::string> sl(8, 0.5f);
...
void test_memory_leak() {
    ...
    assert(NodeBase::active_nodes == 0);

    {
        skiplist<int, int> sl(16, 0.5f);
        for (int i = 0; i < 1000; ++i) {
            sl.insert({i, i});
        }
        assert(sl.size() == 1000);
        // Make sure nodes are allocated
        assert(NodeBase::active_nodes == 1000);
        ...
        assert(sl.size() == 700);
        assert(NodeBase::active_nodes == 700);
        ...
        assert(NodeBase::active_nodes == 500);
...
void test_concurrency() {
...

    {
        ...
        assert(NodeBase::active_nodes == static_cast<int64_t>(sl.size()));
